...
import { AddToCoffeeMenu, AddToCart } from "../../../../state/app.actions";
...
export class CustomizePageComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.store
      .select((x) => ({
        template: x.app.coffeeMenu.find((c) => c.name === template),
      ...
      .subscribe((x) => {
        ...
        const recipe = this.ingredients.map((ing) => {
          const item = templateRecipe.find((r) => r.name === ing) || {
            name: ing,
            quantity: 0,
          };
          return { ...item };
  ...
  addToCart(coffee: any) {
    // actions
    this.store.dispatch(new AddToCoffeeMenu([coffee]));
